...
def set_thumb(post, p):
	...
	if len(res)==0:
		...
		if len(res)>1:
			print("tem muito thumbnail %s" % (post))
		for k in res:
			cursor.execute("UPDATE wp_postmeta set meta_value='%s' where meta_id=%s" % (p, k[0]))
	cnx.commit()
...
for row in r:
	print(row)
	cursor.execute("select * from wp_posts where post_parent=%s and post_mime_type='image/png'" % (row[0]))
	s=cursor.fetchone();
	if s is not None:
		print("FOUND %s" % (s[0]))
		set_thumb(row[0], s[0])
